...
def plot_sensors(
    ...
    for i, sensor in enumerate(sensors):
        ...
        sensor_cut = values[
            (values["timestamp"] >= start) & (values["timestamp"] <= end)
        ]
        sensor_cut["time_relative"] = sensor_cut["timestamp"] - start
        fig.add_trace(
            go.Scatter(
                x=sensor_cut["time_relative"], y=sensor_cut["value"], name=sensor_name
            )
        )
...
def heatmap(
    ...
    for i in range(0, len(sensor_names)):
        ...
        cut_values = values[
            (values["timestamp"] <= end) & (values["timestamp"] >= start)
        ]
...
def compare_tests(
    ...
    for id in config_ids:
        for i, sensor in enumerate(sensors[id]):
            ...
            sensor_cut = values[
                (values["timestamp"] >= starts[id]) & (values["timestamp"] <= ends[id])
            ]
            sensor_cut["time_relative"] = sensor_cut["timestamp"] - starts[id]
            fig.add_trace(
                go.Scatter(
                    x=sensor_cut["time_relative"],
                    y=sensor_cut["value"],
                    name=sensor_name + "_test_" + str(id),
                ),
                row=row,
                col=col,
            )
...
def three_fold_relation(
    ...
    filtered.rename(
        columns={"value": color_sensor, "value_x": x_sensor, "value_y": y_sensor},
        inplace=True,
    )
    joined.rename(
        columns={"value": color_sensor, "value_x": x_sensor, "value_y": y_sensor},
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=filtered[y_sensor],
            mode="markers",
            marker=dict(colorscale="Plasma", showscale=True),
            marker_color=filtered[color_sensor],
            name="Data Points",
        )
    )
...
class Analysis_Instance:
    ...
    def moving_average(self, config_id, windows):
        ...
        for k, sensor in enumerate(self.sensors):
            ...
            for n in windows:
                ...
                fig.add_trace(
                    go.Scatter(
                        x=df["timestamp"] - self.starts[i], y=df[string], name=name
                    )
                )
            fig.update_layout(
                title=sensor, xaxis_title="time in s", yaxis_title=self.yaxis[k]
            )
    ...
    def multiplot_sensors(self, config_id, titles, yaxis, rows, columns, n_matrix):
        subplot_sensors(
            self.dates[config_id],
            self.sensors,
            self.sensors_values[config_id],
            self.starts[config_id],
            self.ends[config_id],
            titles,
            "Time in s",
            yaxis,
            ...
            columns,
            n_matrix,
            self.event_list[config_id],
            self.event_names[config_id],
        )
    ...
    def make_heatmap(self, config_id, title):
        ...
        heatmap(
            ...
            event_names=self.event_names[i],
            title=title,
            titlex="Time in s",
            titley="Sensor",
        )
    ...
    def overlay_tests(self, sync_parameter):
        ...
        for id in self.config_ids:
            for sensor in self.sensors:
                ...
                fig.add_trace(
                    go.Scatter(
                        x=df["timestamp"] - sync_parameter[id][0],
                        y=df["value"].rolling(30).mean(),
                        name=sensor + " test " + str(id),
                    )
                )
            ...
            for j, event in enumerate(self.event_list[id]):
                ...
                fig.add_vline(
                    x=event - sync_parameter[id][0],
                    line_width=1,
                    line_dash="dash",
                    line_color="white",
                    annotation_text=event_name + " test " + str(id),
                )
        fig.update_layout(
            title=sensor, xaxis_title="Time in s", yaxis_title=self.yaxis[0]
        )
        # fig.update_layout(template = 'plotly_dark')
        fig.show()
